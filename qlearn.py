...
for episode in range(num_episodes):
    ...
    for step in range(max_steps_per_episode):
        ...
        if exploration_rate_threshold > exploration_rate:
            ...
            action = q_table[(q_table["x"] == state[0]) & (q_table["y"] == state[1])]["dir"][(q_table[(q_table["x"] == state[0]) & (q_table["y"] == state[1])]["value"]).idxmax()]
        else:
            action = np.random.choice(q_table[(q_table["x"] == state[0]) & (q_table["y"] == state[1])]["dir"])
        ...
        q_table.loc[((q_table["x"] == state[0]) & (q_table["y"] == state[1]) & (q_table["dir"] == action)), "value"] = (1 - learning_rate) * q_table[(q_table["x"] == state[0]) & (q_table["y"] == state[1]) & (q_table["dir"] == action)]["value"] + \
            learning_rate * (reward + discount_rate * np.max(q_table[(q_table["x"] == state[0]) & (q_table["y"] == state[1])]["value"]))
...
rewards_per_hundred_episodes = np.split(np.array(rewards_all_episodes), num_episodes // 1000)
count = 1000
